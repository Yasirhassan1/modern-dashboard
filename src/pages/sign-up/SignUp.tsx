    ...
    return(
        ...
        <div className="card shadow-lg w-fit  rounded-xl p-5 bg-tertiary flex flex-col gap-4 max-w-[400px] flex-1" >
              ...
              <div className="row px-xl-5 px-sm-4 px-3 flex flex-col gap-3 items-center">
                ...
                <div className="mt-2 position-relative text-center w-full">
                  <p className="text-sm font-weight-bold mb-2 text-secondary text-border d-inline z-index-2 bg-tertiary px-3">
                    or
                  </p>
                </div>
              </div>
              <div className="card-body">
                <form action={handleForm} role="form text-left" className="flex flex-col gap-3">
                  <div className="mb-3">
                    <input type="text" required className="form-control p-3 border border-text-secondary rounded-xl w-full text-text-secondary"name="name" placeholder="Name" aria-label="Name" aria-describedby="email-addon"/>
                  </div>
                  <div className="mb-3">
                    <input type="email" required name="email" className="form-control p-3 border border-text-secondary rounded-xl w-full text-text-secondary" placeholder="Email" aria-label="Email" aria-describedby="email-addon"/>
                  </div>
                  <div className="mb-3">
                    <input type="password" required name="password" className="form-control p-3 border text-text-secondary border-text-secondary rounded-xl w-full" placeholder="Password" aria-label="Password" aria-describedby="password-addon"/>
                  </div>
                  <div className="form-check form-check-info text-left flex gap-1.5">
                    <input className="form-check-input" type="checkbox" value="" id="flexCheckDefault" checked={false}/>
                    <label className="form-check-label text-text-secondary">
                      I agree the <a href="javascript:;" className="text-dark font-weight-bolder ">Terms and Conditions</a>
                    </label>
                  </div>
                  <div className="text-center">
                    <button type="submit" className="w-full bg-primary p-3 rounded-lg text-tertiary">Sign up</button>
                  </div>
                  <p className="text-sm mt-3 mb-0 text-text-secondary">Already have an account? <Link to={"/sign-in"} className="text-dark font-weight-bolder font-semibold">Sign in</Link></p>
                </form>
              </div>
            </div>
            </div>
